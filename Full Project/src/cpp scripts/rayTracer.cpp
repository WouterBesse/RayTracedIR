/*
#
# 2021 Jonar Verbart (with help from Wouter M. Besse and Nino Saglia ;)
# Part of RayTracedIR by Wouter M. Besse, Nino Saglia and Jonar Verbart
#
*/

#include "../headers/rayTracer.h"

RayTracer::RayTracer(int SR) {
    sampleRate = SR;
}
...
void RayTracer::rayTrace(Camera *camera, ShapeSet *scene) {
        ...
        for (int j = 0; j < tracedRays[i].size(); j++) {
            if (tracedRays[i][j].rayInBounds) {
                ...
                Vector returnRay = Point(0.0f, 0.0f, 0.0f) -
                                   tracedRays[i][j].pointOfIntersect;
                float totalRayDistance = tracedRays[i][j].summedDistance +
                                         returnRay.length();

                tempSummedD.push_back(totalRayDistance); // vector met floats van bounce i
            }


        }
        raySummedDistances.push_back(tempSummedD); // vector met floats wordt toegevoegd
        tempSummedD.clear();
    }
    ...
    for (int i = 0; i < raySummedDistances.size(); i++) {
        for (int b = 0; b < raySummedDistances[i].size(); b++) {
            ...
            float numSecondsDelay = raySummedDistances[i][b] / 343;
            int numSamplesDelay = sampleRate * numSecondsDelay;
            delayTimesSamples.push_back(numSamplesDelay);
        }
    }
...
std::vector<int> RayTracer::getRaySum() {
    return delayTimesSamples;
}
