...
public:
    ...
    void DrawWireFrameModel(const std::vector<std::pair<float, float>> &vecModelCoordinates, float x, float y,
                            float r = 0.0f, float s = 1.0f, short col = FG_WHITE, short c = PIXEL_SOLID) {
        ...
        for (int i = 0; i < verts; i++) {
            vecTransformedCoordinates[i].first =
                    vecModelCoordinates[i].first * cosf(r) - vecModelCoordinates[i].second * sinf(r);
            vecTransformedCoordinates[i].second =
                    vecModelCoordinates[i].first * sinf(r) + vecModelCoordinates[i].second * cosf(r);
        }
        ...
        for (int i = 0; i < verts + 1; i++) {
            ...
            DrawLine((int) vecTransformedCoordinates[i % verts].first,
                     ...
                     (int) vecTransformedCoordinates[j % verts].second, c, col);
        }
    }
    ...
    void Start() {
        ...
        //mipmap generation
        {
            ...
            for (int level = 4; level < 9; level++) {
                ...
                glTexImage2D(GL_TEXTURE_2D, level, GL_INTENSITY, texsize, texsize, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE,
                             texbuf);
                texsize >>= 1;
            }
    ...
    void GameThread() {
        ...
        while (m_bAtomActive) {
            ...
            while (m_bAtomActive) {
                ...
                swprintf_s(sNewTitle, 256, L"OneLoneCoder.com - Console Game Engine (OGL) - %s - FPS: %3.2f",
                           m_sAppName.c_str(), 1.0f / fElapsedTime);
...
protected: // Audio Engine =====================================================================
    ...
    bool CreateAudio(unsigned int nSampleRate = 44100, unsigned int nChannels = 1,
        ...
        if (waveOutOpen(&m_hwDevice, WAVE_MAPPER, &waveFormat, (DWORD_PTR) waveOutProcWrap, (DWORD_PTR) this,
    ...
    static void
    CALLBACK waveOutProcWrap(HWAVEOUT hWaveOut, UINT uMsg, DWORD dwInstance, DWORD dwParam1, DWORD dwParam2) {
    ...
    float GetMixerOutput(int nChannel, float fGlobalTime, float fTimeStep) {
        ...
        for (auto &s : listActiveSamples) {
            ...
            s.nSamplePosition += (long) ((float) vecAudioSamples[s.nAudioSampleID - 1].wavHeader.nSamplesPerSec *
                                         fTimeStep);

            // If sample position is valid add to the mix
            if (s.nSamplePosition < vecAudioSamples[s.nAudioSampleID - 1].nSamples)
                fMixerSample += vecAudioSamples[s.nAudioSampleID - 1].fSample[
                        (s.nSamplePosition * vecAudioSamples[s.nAudioSampleID - 1].nChannels) + nChannel];
            else
